...
export const decodeCallRequest = (contentMap: IcrcBlob): CallRequest => {
  ...
  const {ingress_expiry, canister_id, sender, ...callRequestRest} = decode<
    Omit<CborCallRequest, 'ingress_expiry'> & {ingress_expiry: {_value: BigNumber} | BigNumber}
  ...
  return {
    ...
    ingress_expiry: BigInt(
      '_value' in ingress_expiry ? ingress_expiry._value.toFixed() : ingress_expiry.toFixed()
    ) as unknown as Expiry
  } as CallRequest;
};
